...
def load_json_file(filename):
    if os.path.exists(filename):
        try:
            ...
            return {} if 'rules' in filename else []
    return {} if 'rules' in filename else []
...
def process_bibtex(input_file, output_file):
    ...
    ignore_file = f"{input_basename}_doi_ignore.json"

    rules = load_json_file(RULES_FILE)
    ignored_dois = load_json_file(ignore_file)
    if not isinstance(ignored_dois, list): ignored_dois = []
    ...
    for entry in bib_database.entries:
        ...
        # Track if we need to save JSON or BIB files after this specific entry
        entry_rules_changed = False
        entry_ignore_changed = False
        entry_bib_changed = False
            ...
            if entry_id not in ignored_dois:
                ...
                if new_doi:
                    ...
                    if new_url:
                        ...
                        print(f"-> Added URL.")
                        url_count += 1
                        ignored_dois.append(entry_id)#!/usr/bin/env python
...
def process_bibtex(input_file, output_file):
    ...
    for entry in bib_database.entries:
        ...
        # If any data was updated for this entry, save BOTH files immediately 
        # so everything is perfectly synced on disk.
        if entry_rules_changed or entry_ignore_changed:
            save_json_file(RULES_FILE, rules)
            save_json_file(ignore_file, ignored_dois)
    ...
    print(f"Ignored DOIs for this paper are stored in '{ignore_file}'.")
